  ...
  implements Flushable
{
  ...
  {
    return 2 * computeTagSize(1) + computeUInt32Size(2, paramInt) + computeBytesSize(3, paramByteString);
  }
    ...
    throws IOException
  {
    ...
    paramByteString.copyTo(this.buffer, paramInt1, this.position, i);
    int j = paramInt1 + i;
    int k = paramInt2 - i;
    ...
    if (k <= this.limit)
    {
      paramByteString.copyTo(this.buffer, j, 0, k);
      this.position = k;
      return;
    }
    InputStream localInputStream = paramByteString.newInput();
    ...
    do
    {
      ...
      if (k <= 0)
        break;
      m = Math.min(k, this.limit);
      n = localInputStream.read(this.buffer, 0, m);
    }
    while (n == m);
